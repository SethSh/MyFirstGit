...
{
    internal class WorkersCompClassCodeProfileRowInserter : BaseRowInserter
    {
        public override bool Validate(ISegmentExcelMatrix excelMatrix, Range range)
        {
            ...
            {
                MessageHelper.Show($"Top selected row can't be on {BexConstants.WorkersCompClassCodeProfileName.ToLower()} bottom row", MessageType.Stop);
                return false;
            }

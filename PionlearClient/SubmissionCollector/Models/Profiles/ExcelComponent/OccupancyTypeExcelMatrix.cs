...
{
    [JsonObject]
    public sealed class OccupancyTypeExcelMatrix : MultipleOccurrenceProfileExcelMatrix, IRangeTransposable
    {
        public OccupancyTypeExcelMatrix(int segmentId) : base(segmentId)
        {
            InterDisplayOrder = 34;
        }

        public OccupancyTypeExcelMatrix()
        {
        ...
        public override string ExcelRangeName => ExcelConstants.OccupancyTypeProfileRangeName;
        ...
        public override bool IsOkToMoveRight
        {
            get
            {
                var maxDisplayOrder = GetSegment().OccupancyTypeProfiles.Max(x => x.IntraDisplayOrder);
        ...
        public override bool IsOkToMoveLeft
        {
            get
            {
                var minDisplayOrder = GetSegment().OccupancyTypeProfiles.Min(x => x.IntraDisplayOrder);
        ...
        {
            var otherExcelMatrix = GetSegment().OccupancyTypeProfiles.Single(x => x.IntraDisplayOrder == IntraDisplayOrder + 1).ExcelMatrix;
        ...
        {
            return GetSegment().OccupancyTypeProfiles.Single(x => x.ComponentId == ComponentId);
        }
        ...
        private StringBuilder ValidateOrdinary()
        {
            ...
            Items = new List<OccupancyTypeDistributionItemPlus>();
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                {
                    var message = $"Enter {BexConstants.OccupancyTypeProfileName.ToLower()} {weightLabel} in {addressLocation}" +
                                  $": <{weightFromExcel}> {BexConstants.NotRecognizedAsANumber}";
                    validation.AppendLine(message);
                    continue;
                }

                var code = OccupancyTypeCodesFromBex.ReferenceData.Single(occupancyType => occupancyType.Name == name.ToString()).Id;
                var item = new OccupancyTypeDistributionItemPlus
                {
                    OccupancyTypeId = Convert.ToInt32(code),
                    Weight = weight,
        ...
        {
            return $"{ExcelConstants.SegmentRangeName}{segmentId}.{ExcelConstants.OccupancyTypeProfileRangeName}{componentId}";
        }

        public static string GetBasisRangeName(int segmentId, int componentId)
        {
            return $"{ExcelConstants.SegmentRangeName}{segmentId}.{ExcelConstants.OccupancyTypeProfileRangeName}{componentId}.{ExcelConstants.ProfileBasisRangeName}";
        }
        ...
        {
            return GetSublinesRangeName(segmentId, componentId, ExcelConstants.OccupancyTypeProfileRangeName);
        }

        public static string GetSublinesHeaderRangeName(int segmentId, int componentId)
        {
            return GetSublinesHeaderRangeName(segmentId, componentId, ExcelConstants.OccupancyTypeProfileRangeName);
        }
    }

...
{
    [JsonObject]
    public sealed class ConstructionTypeExcelMatrix : MultipleOccurrenceProfileExcelMatrix, IRangeTransposable
    {
        public ConstructionTypeExcelMatrix(int segmentId) : base(segmentId)
        {
            InterDisplayOrder = 32;
        }

        public ConstructionTypeExcelMatrix()
        {
                
        }

        [JsonIgnore]
        public IList<ConstructionTypeDistributionItemPlus> Items { get; set; }
        
        public override string FriendlyName => BexConstants.ConstructionTypeProfileName;
        public override string ExcelRangeName => ExcelConstants.ConstructionTypeProfileRangeName;
        ...
        public override bool IsOkToMoveRight {
            get
            {
                var maxDisplayOrder = GetSegment().ConstructionTypeProfiles.Max(x => x.IntraDisplayOrder);
        ...
        public override bool IsOkToMoveLeft {
            get
            {
                var minDisplayOrder = GetSegment().ConstructionTypeProfiles.Min(x => x.IntraDisplayOrder);
        ...
        {
            var otherExcelMatrix = GetSegment().ConstructionTypeProfiles.Single(x => x.IntraDisplayOrder == IntraDisplayOrder + 1).ExcelMatrix;
        ...
        {
            return GetSegment().ConstructionTypeProfiles.Single(x => x.ComponentId == ComponentId);
        }
        ...
        private StringBuilder ValidateOrdinary()
        {
            ...
            Items = new List<ConstructionTypeDistributionItemPlus>();
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                {
                    var message = $"Enter {BexConstants.ConstructionTypeProfileName.ToLower()} {weightLabel} in {addressLocation}" +
                                  $": <{weightFromExcel}> {BexConstants.NotRecognizedAsANumber}";
                    validation.AppendLine(message);
                    continue;
                }
                
                var code = ConstructionTypeCodesFromBex.ReferenceData.Single(ct => ct.SubLineOfBusinessCode == Subline.Code &&  ct.Name == name.ToString()).Id;
                var item = new ConstructionTypeDistributionItemPlus
                {
                    ConstructionTypeId = Convert.ToInt32(code),
                    Weight = weight,
        ...
        {
            return $"{ExcelConstants.SegmentRangeName}{segmentId}.{ExcelConstants.ConstructionTypeProfileRangeName}{componentId}";
        }

        public static string GetBasisRangeName(int segmentId, int componentId)
        {
            return $"{ExcelConstants.SegmentRangeName}{segmentId}.{ExcelConstants.ConstructionTypeProfileRangeName}{componentId}.{ExcelConstants.ProfileBasisRangeName}";
        }
        ...
        {
            return GetSublinesRangeName(segmentId, componentId, ExcelConstants.ConstructionTypeProfileRangeName);
        }

        public static string GetSublinesHeaderRangeName(int segmentId, int componentId)
        {
            return GetSublinesHeaderRangeName(segmentId, componentId, ExcelConstants.ConstructionTypeProfileRangeName);
        }
    }

...
{
    public class ConstructionTypeModel : BaseSourceComponentModel
    {
        ...
        public ConstructionTypeDistributionModel Map()
        {
            return new ConstructionTypeDistributionModel
            {
                Id = SourceId,
                DistributionName = Name,
                SublineIds = SublineIds.Select(id => id.Value).ToList(),
                Items = Items.Select(item => new ConstructionTypeAndWeight
                {
    ...
    internal static class ConstructionTypesExtensions
    {
        public static void ValidateEmptiness(this IList<ConstructionTypeModel> models, StringBuilder validation)
        {
            ...
            {
                validation.AppendLine($"Not every {BexConstants.ConstructionTypeProfileName.ToLower()} contains data");
            }
        }

        public static void ValidateSublineComposition(this IList<ConstructionTypeModel> models, StringBuilder validation, long sublineId, string name)
        {
            ...
            if (hasProfile)
            {
                if (subsetModels.Any())
                {
                    if (subsetModels.Count != 1)
                        validation.AppendLine($"{name} is contained in more than one {BexConstants.ConstructionTypeProfileName.ToLower()}");
                }
                else
                {
                    validation.AppendLine(
                        $"{name} doesn't appear in a {BexConstants.ConstructionTypeProfileName.ToLower()}");
                }
            }
            else
            {
                if (subsetModels.Any())
                    validation.AppendLine(
                        $"{name} can't be contained in a {BexConstants.ConstructionTypeProfileName.ToLower()}");
            }
        }
